...
//The following block was Removed by jingdong    
/*
...
Status DBImpl::WriteToMemtable(const WriteOptions& options,
  ...
  uint64_t sequenceNumber = WriteBatchInternal::Sequence(my_batch);
  Writer* last_writer = &w;
  if (status.ok()) {  // NULL batch is for compactions
    WriteBatch* updates = BuildBatchGroup(&last_writer);
    last_sequence = sequenceNumber 
		    + WriteBatchInternal::Count(updates) - 1;
